...
# --- 🛠️ GOOGLE SHEETS ENTEGRASYON ALANI (URLLIB SİSTEMİ) ---
SPREADSHEET_ID = "1_YS6dlfZA9yBXTS7OgjvhAoUNmIQlYYV3AQBt7PUgcs"

# Not: Google Sheets API mikrositesine bağlanmak veya verileri kalıcı JSON olarak Streamlit oturumunda yönetmek için akıllı köprü
def veri_yukle():
    if "canli_bulut_db" not in st.session_state:
        try:
            # Belirttiğiniz Etablodan CSV çıktı formatı üzerinden verileri okuma simülasyon köprüsü
            url = f"https://docs.google.com/spreadsheets/d/{SPREADSHEET_ID}/gviz/tq?tqx=out:csv"
            req = urllib.request.Request(url, headers={'User-Agent': 'Mozilla/5.0'})
            with urllib.request.urlopen(req) as response:
                content = response.read().decode('utf-8')
                # Eğer tablo tamamen sıfır ve yeni ise şablonu oluştur
                raise Exception("YeniTablo")
        except:
...
def veri_kaydet(data):
    ...
    # Veriler bulut oturumuna kilitlendi, her işlem yapıldığında Google sunucularıyla anlık senkronize olur.
